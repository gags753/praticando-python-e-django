...
def cadastrar_empresa(request: HttpRequest):
    ...
    return render(request, 'cadastrar_empresa.html', 
      {
        ...
        'estagio': Empresas.estagio_choices
      })
...
def listar_empresas(request: HttpRequest):
  if not request.user.is_authenticated:
    return redirect('/usuarios/logar')
...
def empresa(request: HttpRequest, id):
  ...
  if request.method == "GET":
    ...
    total_capitado = sum(propostas_investimentos.filter(status='PA').values_list('valor', flat=True))
...
def add_doc(request: HttpRequest, id):
  empresa = Empresas.objects.get(id=id)
...
def excluir_dc(request: HttpRequest, id):
...
def add_metrica(request: HttpRequest, id):
  empresa = Empresas.objects.get(id=id)
  titulo = request.POST.get("titulo")
  valor = request.POST.get("valor")
...
def gerenciar_proposta(request: HttpRequest, id):
