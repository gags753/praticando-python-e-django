...
def sugestao(request: HttpRequest):
...
def ver_empresa(request: HttpRequest, id):
...
def realizar_proposta(request: HttpRequest, id):
  valor = request.POST.get('valor')
...
def assinar_contrato(request: HttpRequest, id):
